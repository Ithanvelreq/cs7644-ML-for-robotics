...
class FloorPlaneExtract {
    protected:
        ...
        ThumbType check_thumb(const cv::Mat_<cv::Vec3b> & thumb, const cv::Mat_<float> & thumb_z) {
            ...
            int untraversablePixels = 0;
            int traversablePixels = 0;
            for (int r=0;r<thumb_z.rows;r++) {
                for (int c=0;c<thumb_z.cols;c++) {
                    ...
                    if(thumb_z(r,c) > traversability_threshold_){
                        untraversablePixels++;
                    }else{
                        traversablePixels++;
                    }
            ...
            return traversablePixels > untraversablePixels ? TRAVERSABLE:UNTRAVERSABLE;
        }
    ...
    public:
        FloorPlaneExtract() : nh_("~"), it_(nh_) {
            ...
            nh_.param("traversability_threshold",traversability_threshold_,.1);
            nh_.param("purity_threshold",purity_threshold_,.75);
